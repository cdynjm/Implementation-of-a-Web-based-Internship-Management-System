  ...
  <div class="container-fluid {{ (Request::is('static-sign-up') ? 'container' : 'container-fluid') }}">
    ...
    <div class="collapse navbar-collapse" id="navigation">
      <ul class="navbar-nav mx-auto">
        ...
        @endif
        <li class="nav-item">
          <a class="nav-link me-2" @foreach($settings as $set) @if($set->enable_registration == 1) href="{{ auth()->user() ? url('static-sign-up') : url('register') }}" @else href="#" id="registration-disabled" @endif @endforeach>
        ...
        <li class="nav-item">
          <a class="nav-link me-2" href="{{ auth()->user() ? url('static-sign-in') : url('login') }}">
            <i class="fa-solid fa-sign-hanging opacity-6 me-1 {{ (Request::is('static-sign-up') ? '' : 'text-dark') }}"></i>
            Sign In
          </a>
        </li>
      </ul>
      <ul class="navbar-nav d-lg-block d-none">
        <li class="nav-item">
          <a href="#" class="btn btn-sm btn-round mb-0 me-1 bg-gradient-{{ (Request::is('static-sign-up') ? 'light' : 'dark') }}">CCSIT - IMS</a>
        </li>
      </ul>
    </div>
  </div>
